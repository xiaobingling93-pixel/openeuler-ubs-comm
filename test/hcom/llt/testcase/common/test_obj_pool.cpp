...
{
    NResult result;
    NetObjPool<DummyObj> pool("test_pool", 2);
    result = pool.Initialize();
    EXPECT_EQ(result, NN_OK);

    DummyObj *rObj1, *rObj2, *rObj3;
    ...
    pool.Enqueue(rObj2);
    pool.Enqueue(rObj3);
    DummyObj *rObj4, *rObj5;
    ret = pool.Dequeue(rObj4);
    EXPECT_EQ(ret, true);
    EXPECT_EQ(rObj4->tag, 2);
...
{
    std::vector<DummyObj *> objs(11);

    NetObjPool<DummyObj> pool("test_pool", 10);
